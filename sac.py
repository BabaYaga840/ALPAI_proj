...
def evaluate_sac_agent(envs, actor, num_episodes=10):
    ...
    print(f"\nEvaluation: Average Reward: {avg_reward}, Average Goals Reached: {avg_goals_reached}")
...
"""
    ...
    envs = gym.vector.SyncVectorEnv(
        [make_env(args.env_id, args.seed, i, args.capture_video, run_name, args.grid_size) for i in range(args.num_envs)]
    )
    assert isinstance(envs.single_action_space, gym.spaces.Discrete), "only discrete action space is supported"

    # Choose network architecture based on use_simple_network parameter
    if args.use_simple_network:
        actor = SimpleActor(envs, use_uncertainty_map=use_uncertainty_map).to(device)
        actor.load_state_dict(torch.load("checkpoints_True/5/bc_agent_1_99.pth"))
        ...
        actor = Actor(envs, use_uncertainty_map=use_uncertainty_map).to(device)
    ...
    for global_step in range(args.total_timesteps):
        ...
        if global_step < args.learning_starts:
            ...
            # Observations are already on GPU
